...
const { checkRoles, sendMsg, splitMessages, getUserFromMention, checkPermission } = require("./utility");
...
const Timeout = async (msg, client) => {
    try {
        ...
        for (let i = 2; i < args.length; i++) {
            const data = args[i];
            const user = await getUserFromMention(data, client);
